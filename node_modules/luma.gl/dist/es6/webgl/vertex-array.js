...
export default class VertexArray extends Resource {
  ...
  constructor(gl, opts = {}) {
    ...
    this.stubRemovedMethods('VertexArray', 'v6.0', ['setBuffers', 'setGeneric', 'clearBindings', 'setLocations', 'setGenericValues', 'setDivisor', 'enable', 'disable']);
  ...
  setBuffer(locationOrName, buffer, opts = {}) {
    ...
    this.bind(() => {
      ...
      const name = accessInfo ? accessInfo.name : String(location); // Override with any additional attribute configuration params
  ...
  setConstant(locationOrName, arrayValue, opts) {
    ...
    this.bind(() => {
      const location = accessInfo.location;

      this._setConstant(location, arrayValue); // To use the constant value, disable reading from arrays
      // NOTE: Possible perf penalty when disabling attribute 0:
      // https://stackoverflow.com/questions/20305231/webgl-warning-attribute-0-is-disabled-
      // this-has-significant-performance-penalt


      this.gl.disableVertexAttribArray(location); // Reset instanced divisor

      this.gl.vertexAttribDivisor(location, 0); // Save the value for debugging
  ...
  unbindBuffers(disableZero = true) {
    this.bind(() => {
      ...
      this.unbindBuffer = this.unbindBuffer || new Buffer(this.gl, {
        size: 4
      });
  ...
  _getDebugTableRow(attribute, accessor, header) {
    const gl = this.gl; // const round = xnum => Math.round(num * 10) / 10;
