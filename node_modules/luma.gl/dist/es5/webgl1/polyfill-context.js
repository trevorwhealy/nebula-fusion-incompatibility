...
var _polyfillVertexArrayObject = _interopRequireDefault(require("./polyfill-vertex-array-object"));

var _webglRenderingContext = require("./webgl-rendering-context");
...
function isWebGL2(gl) {
  return gl && gl.TEXTURE_BINDING_3D === _constants.default.TEXTURE_BINDING_3D;
...
var WEBGL_CONTEXT_POLYFILLS = (_WEBGL_CONTEXT_POLYFI = {}, _defineProperty(_WEBGL_CONTEXT_POLYFI, OES_vertex_array_object, {
  ...
  getVertexAttrib: function getVertexAttrib(gl, originalFunc, location, pname) {
    ...
    switch (pname) {
      ...
      case _constants.default.VERTEX_ATTRIB_ARRAY_INTEGER:
        result = !webgl2 ? false : undefined;
        break;
      // if instancing is not available, return 0 meaning divisor has not been set

      case _constants.default.VERTEX_ATTRIB_ARRAY_DIVISOR:
  ...
  getProgramParameter: function getProgramParameter(gl, originalFunc, program, pname) {
    if (!isWebGL2(gl)) {
      switch (pname) {
        case _constants.default.TRANSFORM_FEEDBACK_BUFFER_MODE:
          return _constants.default.SEPARATE_ATTRIBS;

        case _constants.default.TRANSFORM_FEEDBACK_VARYINGS:
          return 0;

        case _constants.default.ACTIVE_UNIFORM_BLOCKS:
          return 0;
  ...
  getInternalformatParameter: function getInternalformatParameter(gl, originalFunc, target, format, pname) {
    if (!isWebGL2(gl)) {
      switch (pname) {
        case _constants.default.SAMPLES:
  ...
  getTexParameter: function getTexParameter(gl, originalFunc, target, pname) {
    switch (pname) {
      case _constants.default.TEXTURE_MAX_ANISOTROPY_EXT:
        var extensions = gl.luma.extensions;
        var ext = extensions[EXT_texture_filter_anisotropic];
        pname = ext && ext.TEXTURE_MAX_ANISOTROPY_EXT || _constants.default.TEXTURE_MAX_ANISOTROPY_EXT;
        break;
...
function polyfillContext(gl) {
  (0, _polyfillVertexArrayObject.default)(_webglRenderingContext.WebGLRenderingContext, gl);
  gl.luma = gl.luma || {};
